    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row_index, row in enumerate(reader, 1):
                ...
                reply_txt_name = row.get('REPLY_TEXT', '').strip()
                remble_txt_name = row.get('RUMBLE_TEXT', '').strip()

                guild_id = row.get('GUILD_ID', '').strip()
                channel_id = row.get('CHANNEL_ID', '').strip()
                quantity = row.get('QUANTITY', '').strip()
                timestamp = row.get('TIMESTAMP', '').strip()

                rumble_emoji_id = row.get('RUMBLE_EMOJI_ID', '').strip()
                rumble_emoji_name = row.get('RUMBLE_EMOJI_NAME', '').strip()

                reply_txt_to_send = []
                if reply_txt_name.strip():
                    reply_txt_to_send = read_txt_file(
                        reply_txt_name, f"data/replys/{reply_txt_name}.txt"
                    )

                remble_txt_to_send = []
                if remble_txt_name.strip():
                    remble_txt_to_send = read_txt_file(
                        remble_txt_name, f"data/rembles/{remble_txt_name}.txt"
                    )
                ...
                account = Account(
                    ...
                    reply_txt_to_send=reply_txt_to_send,
                    remble_txt_to_send=remble_txt_to_send,
                    rumble_emoji_id=rumble_emoji_id.strip(),
                    rumble_emoji_name=rumble_emoji_name.strip(),
                )
                accounts.append(account)
        
        logger.success(f"Successfully loaded {len(accounts)} accounts from {file_path}")
        return accounts
