...
from src.model.discord.reply import DiscordReply
from src.model.discord.username import DiscordUsername
from src.model.discord.rumble import DiscordRumble
...
class Start:
    ...
    async def flow(self):
        try:
            ...
            if self.config.TASK == "AI RUMBLE":
                chatter = DiscordRumble(self.account, self.session, self.config)
                await chatter.start_chatting()

            if self.config.TASK == "AI REPLY":
                chatter = DiscordReply(self.account, self.session, self.config)
                await chatter.start_chatting()

            if self.config.TASK == "GET USERNAME":
                chatter = DiscordUsername(self.account, self.session, self.config)
                await chatter.start_chatting()
